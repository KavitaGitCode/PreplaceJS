...
<Answer>
Scope - {} The place in which a variables and functions can be accessed.
...
# What is Prototype?
    ...
    Javascript has default of prototype beheviour.
    Prototype Inheritance...until it gets null.

    "this" set the current context  of object.
...
3.  constructor function is called: the constructor function is called with specified arguments and this is bound to newly createad object. If no explicit return value is specified from the constructor, Javascript assumes this, the newly created object, to be the intended return value.
    in simple word:
    - this (which now refers to the newly created object) will get passed into our function.
    - We can access this through out function using the this keyword.
    - Finally, if there’s no return statement in the constructor function, JavaScript automatically returns the new object.

Prototype: 1. If there is no property directly available in an object then javascript looks up the chain until it finds the property or reaches the end(null)
